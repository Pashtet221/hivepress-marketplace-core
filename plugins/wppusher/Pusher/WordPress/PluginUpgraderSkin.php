...
class PluginUpgraderSkin extends Plugin_Upgrader_Skin
{
    ...
    public function after()
    {
        ...
        if ( ! is_null($this->error)) {
            ...
            do_action('wppusher_plugin_update_failed', new PluginUpdateFailed($message));
    ...
    private function decorateIncompatibleArchive(WP_Error $error)
    {
        ...
        if ($type === 'bb') {
            ...
            // For non-Bitbucket packages (or unknown), surface generic guidance
            // instead of misleadingly blaming a Bitbucket token.
    ...
    {
        if ( ! isset($this->upgrader) || ! isset($this->upgrader->plugin)) {
            return null;
        }
        $plugin = $this->upgrader->plugin;
        if ( ! isset($plugin->repository) || ! isset($plugin->repository->lastAuthError)) {
            return null;
        }
        return $plugin->repository->lastAuthError ?: null;
    }

    private function getRepositoryCode()
    {
        if (isset($this->upgrader, $this->upgrader->plugin, $this->upgrader->plugin->repository->code)) {
            return $this->upgrader->plugin->repository->code;
        }
    ...
    private function logError($code, $message)
    {
        ...
        $logger->error("Plugin install/update failed ({$code}): {$plain}");
    }
