...
if( !class_exists('WPUpdatesPluginUpdater_957') ) {
    class WPUpdatesPluginUpdater_957 {
        ...
        function check_for_update($transient) {
            ...
            $url = $this->api_url . '?v=' . $transient->checked[$this->plugin_path];

            // Identify the site on the existing update check so the license
            // server can record that this license is still actively installed.
            $license_key = get_option('wppusher_license_key');

            if ($license_key) {
                $url .= '&key=' . urlencode($license_key) . '&site=' . urlencode(home_url());
            }
